...
// Events generated by GCodeMachineControl.
// We might want a separate event receiver as some status events
// are directly handled by machine control. For instance, pause and resume.
// Or other internal states independent of the parser.
class GCodeMachineControl::StatusEventReceiver {
public:
  virtual ~StatusEventReceiver() {}

  // GCode Machine control has been initialized event.
  virtual void GCodeMachineControlCreated(GCodeMachineControl *machine_control) {}

  virtual void enqueued_line_move(float feed_mm_p_sec, const AxesRegister &absolute_pos) = 0;
